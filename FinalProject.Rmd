...
```{r, fig.cap="News media sentiment from 1990 to 2021 based on headlines with positive (green), negative (red), and neutral (grey) sentiment."}
ggplot(data = sent_df_group, aes(x = year, y = n, color = polarity)) + 
  geom_line(size = 1.5) + 
  labs(y = "Number of Headlines",
       x = "Year",
       color = "Sentiment", 
       title = "Developed Media Sentiment on IPM") + 
...
```{r}
ggplot(data = slr_nrc_sent2, aes(x = year, y = percentage, color = sentiment)) + 
